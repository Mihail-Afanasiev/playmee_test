...
package com.xtdstudios.DMT.atlas
{
	...
	public class AtlasGenerator implements IRunnable
	{
		...
		private static const TEXTURES_EXTRUDE	: int = 2;
		...
		private function extractRectangles():void
		{
			...
			{
				rect = new Rectangle(0, 0, capturedAsset.bitmapData.width + 2 * TEXTURES_EXTRUDE, capturedAsset.bitmapData.height + 2 * TEXTURES_EXTRUDE);
		...
		private function copyBitmaps():void
		{
			...
			for each (var rectKey:Rectangle in solvedRectangles) 
			{
				...
				//left extrude
				var i:int, j:int;
				for (i = 0; i < TEXTURES_EXTRUDE; i++)
					atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, 0, 1, bitmapData.height), new Point(rect.x + i, rect.y + TEXTURES_EXTRUDE));
				//top extrude
				for (i = 0; i < TEXTURES_EXTRUDE; i++)
					atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, 0, bitmapData.width, 1), new Point(rect.x + TEXTURES_EXTRUDE, rect.y + i));
				
				atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, 0, bitmapData.width, bitmapData.height), new Point(rect.x + TEXTURES_EXTRUDE, rect.y + TEXTURES_EXTRUDE));
				
				//right extrude
				for (i = 0; i < TEXTURES_EXTRUDE; i++)
					atlasBitmapData.copyPixels(bitmapData, new Rectangle(bitmapData.width - 2, 0, 1, bitmapData.height), new Point(rect.x + bitmapData.width + TEXTURES_EXTRUDE + i - 1, rect.y + TEXTURES_EXTRUDE));
				//bottom extrude
				for (i = 0; i < TEXTURES_EXTRUDE; i++)
					atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, bitmapData.height - 2, bitmapData.width, 1), new Point(rect.x + TEXTURES_EXTRUDE, rect.y + bitmapData.height + TEXTURES_EXTRUDE + i - 1));
					
				//corners
				for (i = 0; i < TEXTURES_EXTRUDE; i++)
				{
					for (j = 0; j < TEXTURES_EXTRUDE; j++)
					{
						atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, 0, 1, 1), new Point(rect.x + i, rect.y + j));
						atlasBitmapData.copyPixels(bitmapData, new Rectangle(0, bitmapData.height - 2, 1, 1), new Point(rect.x + i, rect.y + TEXTURES_EXTRUDE + bitmapData.height + j - 1));
						atlasBitmapData.copyPixels(bitmapData, new Rectangle(bitmapData.width - 2, 0, 1, 1), new Point(rect.x + TEXTURES_EXTRUDE + bitmapData.width + i - 1, rect.y + j));
						atlasBitmapData.copyPixels(bitmapData, new Rectangle(bitmapData.width - 2, bitmapData.height - 2, 1, 1), new Point(rect.x + TEXTURES_EXTRUDE + bitmapData.width + i - 1, rect.y + TEXTURES_EXTRUDE + bitmapData.height + j - 1));
					}
				}
				...
				var realRect:Rectangle = new Rectangle(rect.x + TEXTURES_EXTRUDE, rect.y + TEXTURES_EXTRUDE, rect.width - 2 * TEXTURES_EXTRUDE, rect.height - 2 * TEXTURES_EXTRUDE);

				resultAtlas.addRegion(capturedAsset.id, realRect);
